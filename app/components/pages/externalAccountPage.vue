<template>
  <page actionBarHidden="true" :class="{ 'dark': darkModeToggle }">
    <StackLayout>
      ...
      <FlexboxLayout class="actionButton" @tap="addFriend" v-if="(!loading && !error) && localUserData.id != userid && userdata.friend == 'FALSE'">
        <Label col="0" text.decode="&#xf234;" class="fas" style="margin-right: 15px"></Label>
        <Label>Aggiungi {{ userdata.name }} come amico</Label>
      </FlexboxLayout>
      <FlexboxLayout class="actionButton gray" @tap="addFriend" v-else-if="(!loading && !error) && localUserData.id != userid && userdata.friend == 'PENDING'">
        <Label col="0" text.decode="&#xf234;" class="fas" style="margin-right: 15px;"></Label>
        <Label>Richiesta inviata</Label>
      </FlexboxLayout>
      <FlexboxLayout class="actionButton red" @tap="addFriend" v-else-if="(!loading && !error) && localUserData.id != userid && userdata.friend == 'TRUE'">
        <Label col="0" text.decode="&#xf503;" class="fas" style="margin-right: 15px;"></Label>
        <Label>Rimuovi amico</Label>
      </FlexboxLayout>
      <FlexboxLayout class="actionButton blue" @tap="addFriend" v-else-if="(!loading && !error) && localUserData.id != userid && userdata.friend == 'REQUESTED'">
        <Label col="0" text.decode="&#xf14a;" class="fas" style="margin-right: 15px;"></Label>
        <Label>Accetta richiesta di amicizia</Label>
      </FlexboxLayout>
